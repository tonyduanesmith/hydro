...
const Graph = (label, data) => {
  ...
  const spaces = Array(COLUMNS - 4)
    .fill(" ")
    .join("");
  const labelSpaces = Array(COLUMNS - 4 - label.length)
    .fill(" ")
    .join("");
  ...
  const minLabelSpaces = Array(COLUMNS - 4 - minLabel.length)
    .fill(" ")
    .join("");
  const maxLabelSpaces = Array(COLUMNS - 4 - maxLabel.length)
    .fill(" ")
    .join("");
